...
func (r *categoryResolver) Cursos(ctx context.Context, obj *model.Category) ([]*model.Course, error) {
	cource, err := r.CourseDB.FindByCategoryId(obj.ID)
	if err != nil {
		return nil, err
	}

	var cources []*model.Course

	for _, item := range cource {
		cources = append(cources, &model.Course{
			ID:          item.ID,
			Name:        item.Name,
